"""
MicroPython driver for lps22 chip
...
Example usage on RP Pico:
    ...
    while True:
        ...
        temperature = lps22hb.read_temperature()
...
class LPS22HB:
    def __init__(self, i2c: I2C, address=LPS22HB_I2C_ADDRESS):
    ...
    def read_pressure(self) -> float:
        if (self._read_byte(LPS_STATUS) & 0x01) == 0x01:  # a new pressure data is generated
            byte0=self._read_byte(LPS_PRESS_OUT_XL)
            byte1=self._read_byte(LPS_PRESS_OUT_L)
            byte2=self._read_byte(LPS_PRESS_OUT_H)
            return ((byte2 << 16) + (byte1 << 8) + byte0) / 4096.0
        else:
            return 0.0

    def read_temperature(self) -> float:
        if (self._read_byte(LPS_STATUS) & 0x02) == 0x02:   # a new pressure data is generated
            byte0 = self._read_byte(LPS_TEMP_OUT_L)
            byte1 = self._read_byte(LPS_TEMP_OUT_H)
            return ((byte1 << 8) + byte0) / 100.0
        else:
            return 0.0
