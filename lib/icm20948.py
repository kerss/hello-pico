"""
MicroPython driver for icm20948 chip
Author: ?

Example usage on RP Pico:
    i2c = I2C(1, scl=Pin(15), sda=Pin(14))
    imu = ICM20948(i2c=i2c, i2c_addr=I2C_ADDR)
    ...
    while True:
        ...
        print(f"Accel:\t{ax:05.2f} {ay:05.2f} {az:05.2f}")
        print(f"Gyro:\t{gx:05.2f} {gy:05.2f} {gz:05.2f}")
        print(f"Mag:\t{mx:05.2f} {my:05.2f} {mz:05.2f}")
        print(f"Temp:\t{temp:05.2f}")
...
class ICM20948:
    def __init__(self, i2c:I2C, i2c_addr=I2C_ADDR):
        self._bank = -1
        self._addr = i2c_addr
        self._i2c = i2c
    ...
    def quick_read(self):
        self.bank(0)
        data = self.read_bytes(ICM20948_ACCEL_XOUT_H, 6)

        ax, ay, az = struct.unpack(">hhh", bytearray(data))

        return ax / 2048.0, ay / 2048.0, az / 2048.0
